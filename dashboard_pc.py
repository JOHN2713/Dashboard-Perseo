...
st.set_page_config(
    page_title="Dashboard Perseo PC | Perseo",
    page_icon="💻",
    layout="wide",
...
def cargar_datos():
    ...
    file_id = "168V0BCy0-LZDzeKQp-9sdJuXIbRQvOpd"
    
    try:
        st.sidebar.info("📡 Descargando datos desde Google Drive...")
        
        # Descargar archivo temporalmente
        temp_file = "temp_pc.xlsx"
        ...
        try:
            ...
            df = pd.read_excel('BASE PRODUCTO PC.xlsx')
    ...
    df['Fecha Actualizaciones'] = pd.to_datetime(df['Fecha Actualizaciones'], errors='coerce')
    # Rellenar valores nulos en Producto
...
with col_left2:
    ...
    ingreso_activo_producto = df_activos.groupby('Producto')['Ingreso Mensual Equiv'].sum().reset_index()
    
    fig5 = px.pie(
        ingreso_activo_producto,
        values='Ingreso Mensual Equiv',
        names='Producto',
...
columnas_importantes = [
    ...
    'Ingreso Mensual Equiv', 'Inicio', 'Vence', 'Año Inicio',
    'Meses Vigencia', 'Empresas Activas', 'Empresas Inactivas',
    'Usuarios Activos', 'Provincia'
]
...
if columnas_seleccionadas:
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f'perseo_pc_filtrado_{datetime.now().strftime("%Y%m%d")}.csv',
        mime='text/csv'
    )
else:
...
<div style='text-align: center; padding: 2rem 0;'>
    ...
    <p style='color: gray; margin: 0;'>Dashboard Perseo PC - Análisis de Clientes y Suscripciones</p>
